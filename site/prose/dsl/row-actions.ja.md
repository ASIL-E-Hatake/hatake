...
行のボタンは**行の右端に、並べた順に**出る。`search` でも `crud` でも `master` でも同じで、書き方も同じ（`rowActions` に id を並べるだけ）。`crud` / `master` では組み込みの `edit` / `delete` も同じ列に混ざる。

```yaml
table:
  rowActions: [detail, edit, delete]   # この順に出る
```

押した行はハンドラに渡る（`ctx.record`）。`navigate` なら `params` に `$row.<項目名>` で行の値を差し込める。

## 一括のボタンは行に並べられない

`scope: selection` のボタン（→ [一括のボタン](/dsl/bulk-actions)）を `rowActions` に並べても、**行には出ない**。

行のボタンは「押した行」に実行するもので、一括のボタンは「チェックした行」に実行するもの。行に並べると、押した行ではなくチェックした行に実行することになる ── 何もチェックしていなければ**押しても何も起きない**。一番静かな壊れ方なので、行には出さず一覧の上の一括ボタンとして出す（行から消してしまうと、どこからも押せないボタンになる）。

並べても壊れないが、効いていないので `npx hatake validate` が言う（`selection-as-rowaction`）。読み返し（`npx hatake explain`）も「行には出ない：選んだ行に実行するボタン」と書く。
