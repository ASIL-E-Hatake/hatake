...
**誰でもない人（未ログイン）の列が必ず入る。** 役割を持つ人だけを並べると、ログインしていない人に何が見えているかを誰も見ないままになる（デモで切り替えられるようにして最初に分かったのがそれ）。

並べているのは**役割で絞られている物だけ**。絞っていない物は全員に見えるので、並べると表が定義の大きさになって読まれない。

「画面」の行は**入口を辿った結果**。ページ自身に `roles` は書けないので、メニューやボタンの権限から決まる（メニューの行は同じことを言うので出さない）。

`--role` に定義のどこにも出てこない役割名を渡すと**エラー**にする。黙って0枚を返すと「その役割では何も開けない」と読めてしまうので。

`explain --roles` の役割ごとの行にも「**見えるのは何件・見えないのは何件**」が出る。「この役割で何ができるか」は、**見えない側**まで書かないと答えにならない。

### アプリが配るのに、使っていない役割

`--registry` を渡すと逆の抜けも言う。

```
アプリが配るのに、定義が出し分けに使っていない役割 … viewer
```

**消す相手ではない。** アプリのコードで使っていることもあるし、出し分けの書き忘れのこともある。言えるのは「この役割で見え方が変わる所は1つもない」という事実だけ。逆向き（定義にしか無い役割）は `validate --registry` が警告で言う。
...
`roles` に書くのは自分で決めた文字列なので、**綴りを間違えても画面は出る**（その列やボタンが誰にも見えなくなるだけ）。画面を見ても「そういう仕様」に見えるので、いちばん気づきにくい。

`app.roles` に**配りうる役割の全部**を並べておくと、そこは機械が言う。

```yaml
app:
  id: hr_admin
  title: 人事管理
  roles: [hr, manager, executive]
```
...
**並べても何も与えない。** 出し分けは各所の `roles`、実際の遮断はサーバ。ここは語彙の宣言で、権限の設定ではない。

書かなければ今までどおりで、この検査はアプリ側の一覧（`HatakeScope(knownRoles:)`／`validate --registry`）が要る。つまり `app.roles` の値打ちは**定義1枚で閉じること** ── AI に定義だけ渡したときに、綴りを自分で確かめられる。

逆向き（宣言したのに、どこにも出し分けに使っていない役割）も `explain --roles` が言う。これも消す相手ではなく、「この役割で見え方が変わる所は1つもない」という事実。

## デモで切り替えて見る

権限は**見えないことが正しい**機能なので、書いてあるだけでは伝わらない。<a href="/hatake/demo/" target="_self">デモ</a>の右下に「見ている人」の札があり、そこで切り替えると同じ画面が変わる。

| 見ている人 | 受注照会で何が変わるか |
| --- | --- |
| 担当（`staff`） | 粗利の列は出ない・却下のボタンも出ない・原価管理はメニューに無い |
| 承認者（`staff` ＋ `manager`） | 粗利の列と却下が出る・原価管理の入口が出る・一括承認は 50 件まで（担当は 20 件） |
| 誰でもない（未ログイン） | CSV出力が出ない（持ち出しはログインした人だけ） |

**切り替えても開いている画面はそのまま**なので、同じ一覧で「何が消えたか」を見比べられる。役割はアプリが配るもの（ログイン状態）なので、この札は定義の側ではなくデモアプリ自身の作り。

承認者が担当の役割も持っている（`{staff, manager}`）のは、**役割が足し算**だから。「承認者だから担当の画面が見えない」ことにはならない。
