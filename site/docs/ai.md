...
エージェントに渡る道具は18個。どれにも**そのまま渡せる引数の例**が1組付いていて（説明の最後に出る）、その例は CI が実際に呼んで通ることを見ている＝**古い例が残らない**（例を写して最初の1回で転ぶ、が起きない）。

| 道具 | 何をするか |
| --- | --- |
| `hatake_project` | **いちばん最初**。案件の前書き（何のシステムか・**何ができないか**・業務の言葉と項目名の対応・名前の決めごと）を読む。書いてある言葉と名前で定義を書く → [AI への頼み方](/asking) |
