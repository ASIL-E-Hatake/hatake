...
export const WARNING_RULES: Record<string, RuleDoc> = {
  ...
  "key-duplicate-field": {
    what: "同じ項目を2回書いた複合キー",
    happens:
      "複合キーは項目名で束ねるので、同じ名前を2回書いても鍵は1つ分にしかなりません。"
      + "一方で**出す API の道は書いた数だけ区切りを宣言する**ので、"
      + "宣言と実際がずれます（サーバは在りもしない道を受け取ります）。",
    fix: "重なっているほうを消してください（1件を決めるのに要る項目だけを、要る順に）。",
  },
  "key-single-in-list": {
    what: "項目が1つしかない複合キー",
    happens:
      "並びで書いてあるのに項目が1つ＝複合キーではありません。動きますが、"
      + "読む人に「2つ以上ある」と誤解させます。",
    fix: "項目が1つなら文字で書いてください（`key: orderNo`）。",
  },
  "key-empty": {
    what: "空の key",
    happens:
      "`key: []` は1件を指す項目が無いので、**既定の `id` で動きます**＝"
      + "書いたことは効きません。`id` という項目が無ければ、開く・直す・消すが動きません。",
    fix: "1件を決める項目を書いてください（`key: orderNo` か `key: [orderNo, lineNo]`）。",
  },
...
export const ADVICE_RULES: Record<string, RuleDoc> = {
  ...
  "navigate-without-key-param": {
    what: "鍵を渡していない画面移動",
    happens:
      "1件を読む・直す画面は**その画面の `key` の名前**（書いていなければ `id`）で"
      + "鍵を受け取ります。違う名前で渡すと、URL は変わるのに**開いた画面は空**になり、"
      + "データを取りに行きもしません。エラーも出ないので、押した人には「壊れている」"
      + "としか見えません。",
    fix:
      "`params` に行き先の `key` と同じ名前を入れてください"
      + "（一覧の行から渡すなら `{ <key>: $row.<key> }`）。",
  },
  "detail-page-in-menu": {
    what: "メニューに直接置いた詳細画面",
    happens:
      "メニューには**鍵を渡す場所がありません**。1件を読む画面（`type: detail`）を"
      + "メニューから開くと、必ず「データがありません」になります。",
    fix:
