...
function checkComputed(
  ...
  if (target === undefined) {
    ...
    // **畳む相手がどちらも無い。** `sum` のように両方のモードに在る op は、
    // `field`（明細の行）も `fields`（同じレコードの項目）も無いまま素通りしていた。
    // 結果は空欄ではなく **0**＝請求書の金額が黙って 0 円になる形で出る。
    if (list(computed.fields).length === 0) {
      warn(
        found,
        "computed-without-source",
        `${at}`,
        `${op} は何かを畳む計算ですが、**畳む相手がありません**` +
          "（`fields` も `field` も書かれていません）。" +
          `「${label}」は空欄ではなく**${op === "concat" ? "空文字" : "0"}**になります` +
          "＝転んだことに気づけません。",
        of === undefined
          ? "同じレコードの項目を足すなら `fields: [項目名, …]`、" +
            "明細の行を畳むなら `field: <明細の項目名>` を足してください。"
          : "`of` を書いているので、**明細の行を畳む**つもりだと思います。" +
            "`field: <明細の項目名>` を足してください" +
            "（同じレコードの項目を足すなら、`of` ではなく `fields` です）。",
      );
      return;
    }
