// **内部の口**（`@hatake-fw/api/internal`）。
//
// ここから出ているものは**約束しません**。名前も形も、断りなく変わります。
// 枠組みの中身そのものなので、直したいときに直せる必要があるからです。
//
// 約束するのは `@hatake-fw/api`（`index.ts`）から出ている分だけです。
// 一覧は [`spec/public-api.ts.json`](../../spec/public-api.ts.json) に在り、
// 試験が完全一致を見ます。
//
// **なぜ2つに分けたか。** `index.ts` が 142 モジュールを `export *` で丸ごと出して
// いたので、**内部に定数を1つ足すだけで公開 API が1つ増えて**いました（8日で
// 586 → 588、どちらも検証規則の内部の表）。全部を約束すると、人に見せる字を
// 良くするだけで「約束を破った」ことになります。
//
// かといって隠してしまうと、枠組みの中身を使いたい人の道が消えます。だから
// **道は残して、約束だけを分け**ました。

export * from "./definition.js";
export * from "./parse.js";
export * from "./strictKeys.js";
export * from "./reference.js";
export * from "./fragmentContext.js";
export * from "./specDir.js";
export * from "./mcpTools.js";
export * from "./mcpContract.js";
export * from "./scaffold.js";
export * from "./examples.js";
export * from "./pitfalls.js";
export * from "./failures.js";
export * from "./actionNeeds.js";
export * from "./warnings.js";
export * from "./wire.js";
export * from "./refs.js";
export * from "./registryScan.js";
export * from "./registryDiff.js";
export * from "./appParse.js";
export * from "./validators.js";
export * from "./formValidator.js";
export * from "./scenario.js";
export * from "./scenarioDraft.js";
export * from "./scenarioCover.js";
export * from "./fieldValues.js";
export * from "./fixtures.js";
export * from "./intent.js";
export * from "./intentDraft.js";
export * from "./trace.js";
export * from "./project.js";
export * from "./projectAdvise.js";
export * from "./projectAgents.js";
export * from "./responsibility.js";
export * from "./probeHelp.js";
export * from "./projectCoverage.js";
export * from "./projectDraft.js";
export * from "./projectDrift.js";
export * from "./questionImpact.js";
export * from "./questions.js";
export * from "./questionTriggers.js";
export * from "./actionFallbacks.js";
export * from "./projectExplain.js";
export * from "./messageResolver.js";
export * from "./conditionEvaluator.js";
export * from "./computed.js";
export * from "./options.js";
export * from "./access.js";
export * from "./aggregate.js";
export * from "./csv.js";
export * from "./papers.js";
export * from "./placeholders.js";
export * from "./printLayout.js";
export * from "./printMetrics.js";
export * from "./printStyle.js";
export * from "./reportLayout.js";
export * from "./paperText.js";
export * from "./sampleRows.js";
export * from "./report.js";
export * from "./query.js";
export * from "./dslVersion.js";
export * from "./dto.js";
export * from "./dtoDiff.js";
export * from "./explain.js";
export * from "./explainAccess.js";
export * from "./explainRoles.js";
export * from "./explainPhrases.js";
export * from "./explainVoice.js";
export * from "./explainSource.js";
export * from "./explainDiff.js";
export * from "./explainMarkdown.js";
export * from "./gitRange.js";
export * from "./explainBrief.js";
export * from "./harvest.js";
export * from "./shrink.js";
export * from "./minimize.js";
export * from "./repro.js";
export * from "./yamlSpans.js";
export * from "./yamlWrite.js";
export * from "./diagnose.js";
export * from "./fix.js";
export * from "./advise.js";
export * from "./bulkLimit.js";
export * from "./adviseApply.js";
export * from "./adviseDraft.js";
export * from "./pageParts.js";
export * from "./roleBulk.js";
export * from "./roles.js";
export * from "./roleSight.js";
export * from "./roleMatrix.js";
export * from "./adviseCompare.js";
export * from "./adviseRules.js";
export * from "./adviseRequire.js";
export * from "./review.js";
export * from "./adviseOff.js";
export * from "./design.js";
export * from "./harvestRules.js";
export * from "./screenIndex.js";
export * from "./diagram.js";
export * from "./appDiagram.js";
export * from "./graphText.js";
export * from "./computedGraph.js";
export * from "./appAccess.js";
export * from "./defDiff.js";
export * from "./jsonSchema.js";
export * from "./openApi.js";
export * from "./wireKinds.js";
export * from "./wireMerge.js";
export * from "./wireTodo.js";
export * from "./wiringFilled.js";
export * from "./registryUse.js";
export * from "./registryFromApp.js";
export * from "./wiringGaps.js";
export * from "./adviseEffect.js";
export * from "./checkSheet.js";
export * from "./renameDraft.js";
export * from "./wizardSteps.js";
export * from "./warningRules.js";
export * from "./rules.js";
export * from "./ruleCases.js";
export * from "./scenarioCoverDraft.js";
export * from "./same.js";
export * from "./widgetDraft.js";
export * from "./traceDiff.js";
export * from "./httpProbe.js";
export * from "./restTarget.js";
export * from "./probeShape.js";
export * from "./probe.js";
export * from "./attack.js";
export * from "./attackSweep.js";
export * from "./login.js";
export * from "./loginRun.js";
export * from "./runSnapshot.js";
export * from "./runDiff.js";
export * from "./types.js";
export * from "./formatter.js";
export * from "./converter.js";
export * from "./normalizer.js";
export * from "./tax.js";
export * from "./fiscal.js";
export * from "./age.js";
export * from "./businessDay.js";
export * from "./era.js";
