...
**読めない版は落とす。** 黙って今の版として読むのがいちばんまずい＝将来 `2.0` を出したとき、
世に出た `1.x` の道具が `2.0` の定義を**画面は出るのに意味が違う**形で読む。しかも読めて
しまうので、誰も気づけない。

| 書いてある字 | どうなる | なぜ |
|---|---|---|
| 無し | 通す（既定 `1.0`） | 書かなくていい。ここを落とすと既にある定義が全部落ちる |
| `1.0` | 通す | |
| `1.1`（同じ major の新しい minor） | **読むが1件言う**（`dsl-version-newer`） | minor は**足すだけ**と決めているので読める。読めないキーは strict の担当 |
| `2.0` / `0.9`（知らない major） | **落とす** | 別の版の定義を今の版として読まない |
| `1` / `1.0.0` / `v1.0` / 空 / 前後に空白 | **落とす** | 推し量らない。`1` を `1.0` と読むと、次に `1` の意味を決められなくなる |

判定は**3版（Dart / TypeScript / Java）で同じ**で、JSON Schema も同じものを弾く
（`dsl_version` は `^1\.[0-9]+$`）。正は
[`spec/conformance/dsl_version.json`](conformance/dsl_version.json)で、印（`kind`）まで
固定してある＝文面は版ごとの言葉でよいが、「通す／言う／落とす」がズレたら、同じ定義が
版によって通ったり落ちたりする。

minor は**足すだけ**（キー・値・既定値を増やす）と決めている。**意味を変える・消す**ときは
major を上げる。→ [1.0 の約束](../docs/compat.ja.md)
...
`on` / `off` / `yes` / `no` / `true` / `false` / `null` / `~` は**キー名にできない**。
YAML には読み手が2つあって、解釈が違うから。

| 読み手 | `on:` をどう読むか |
| --- | --- |
| YAML 1.2（TypeScript の `yaml`） | 文字の `"on"` |
| YAML 1.1（PyYAML＝スキーマ検証） | **真偽値の `true`** |

つまりこの名前を使うと、**片方の読み手でしか通らない紙**ができる（TS では書けるのに、
スキーマ検証は「知らないキー `True`」で落ちる）。実際に落としたので、いまは**同梱の
スキーマにこの名前が無いこと**を試験が見ている。値として書くときも同じ語を引用符で
囲む（書き出す側が自動でやる）。
...
`rowActions` はページレベルのアクション（例: `detail`）を指して、対象行を context に乗せて呼ぶ。
例: [`examples/product_search.yaml`](examples/product_search.yaml)。

**行のボタンは、どの種別でも同じ所（行の右端）に出る。** 書き方も同じ（`table.rowActions` に
id を並べる）。`crud` / `master` では組み込みの `edit` / `delete` も同じ列に混ざり、出る順は
並べた順。
...
| **行には並べられない**（`table.rowActions`） | 行のボタンは押した行に実行するもの。一括のボタンを並べると、押した行ではなく**チェックした行**に実行することになる（何も選んでいなければ何も起きない）。並べても行には出ず、一覧の上の一括ボタンとして出る＝`validate` が `selection-as-rowaction` と言う |
...
行を名指しすると4つが付いてくる。文言の `{failedKeys}` が埋まる・失敗の通知から
**「どの行か」**を開ける（キーと理由を1件ずつ）・そこから**その行だけを選び直せる**
（もう一度押す相手を人が選び直さなくていい）・**CSV に持ち出せる**（後述）。名指し
しなければ件数だけの報告として扱われ、`{failedKeys}` は文字のまま出る（「行が
分かっていない」と読める）。
...
| `rowActions` | string[] | `[]` | 行ごとのアクションid（行の右端に、並べた順に出る）。組込: `edit`, `delete`（`crud` / `master` だけ）。**選んだ行に実行するボタンは並べられない**（→ [まとめて実行する](#まとめて実行するscope-selection)）。 |
...
**残った行は、画面の外へ持ち出せる。** 選び直しは「いま・ここ」の話で、読み直す・
ページを変える・画面を閉じるで消える。一括の失敗と中断はそこで終わりではなく（担当に
配る・翌日やり直す）続きが在るので、**CSV に出す**口を通知とダイアログに付けてある
（出す口＝`exportSink` を登録しているときだけ出る＝押しても何も起きないボタンを作らない）。

- 出すのは**1枚**（失敗した行と実行していない行で分けない＝次にやることは同じ）。
  代わりに**理由の列**を足して、行ごとに「失敗の理由」か「実行していません」を書く
- 列は表の列（**その人に見えている列だけ**。`type: export` と同じ扱い）＋理由
- 区切りが投げて終わったときも、**そこまでに名指しされた失敗**は紙に残る
  （投げた区切りより前の報告を落とさない）
...
| 入力する画面（`form` / `wizard`）のボタン | **いま入力されている値**（保存前。計算した項目も含む） |
| 読むだけの画面（`detail`）のボタン | いま開いているレコード |
...
- **0 件のときも、なぜ押せないのかがボタンに出る。** 「一括承認（行を選んでください）」＝
  行は在るがまだ選んでいない／「一括承認（行がありません）」＝絞り込んだ結果が空で、選べる
  行が無い（直し方が違うので言い分ける）。読み込んでいる間は何も足さない＝出てくる行を
  待っている間だけ嘘になるので
...
- **入力する画面では、保存を挟まずに変わる。** 「下書きに直したら送信できる」が書ける＝
  項目の側（`visibleWhen` / `computed`）が見ているものと**同じ値**でボタンも判定する
  （保存しないと押せないボタンは、直したのに灰色のまま見える）
- 入力する画面では `{ mode: create }` / `{ mode: edit }` も判定できる（新規入力のときだけ
  押せる「下書き保存」が書ける）。読むだけの画面はフォームの状態を持たないので、
  `mode` のリーフは満たされない
- 押せない理由に出る項目名は、**その画面の見出し**で言う（`status` ではなく「状態」）
...
| `selection-as-rowaction` | `scope: selection` のボタンの id を `table.rowActions` に並べた → **行には出ない**（行のボタンは押した行に実行するもの。一覧の上の一括ボタンとして出る） |
...
`validate` が言うのは「**書ける**」ことだけ、`explain` が言うのは「**そう書いてある**」
ことだけ。その値でいくらになるか・その状態で何が必須になるか・そのボタンが押せるかは、
**動かさないと分からない**。
...
返るのは5つ。**答えの作り方は画面と同じ順**（`normalize` → `computed` → 状態 → 検証）で、
ここがズレると道具の答えが嘘になる。

| 欄 | 何が返るか | 期待の見方 |
|---|---|---|
| `errors` | 検証エラー（`<項目>[<行>].<行の項目>` も同じ形） | 書いたら**順不同で完全一致**（`[]` は「エラー無し」） |
| `computed` | 計算した値（行の中の計算も当たったあと） | 書いた**キーだけ** |
| `enabled` | ボタンが押せるか（`enabledWhen`） | 書いた**キーだけ** |
| `hidden` | 隠れている項目（`visibleWhen`・枠の `visibleWhen`） | 書いたものが**入っていること** |
| `required` | いま必須の項目（`required` と成立した `requiredWhen`。**隠れている項目は数えない**） | 書いたものが**入っていること** |

欄ごとに見方が違うのは、欄の形が違うから（エラーは「これだけ出る」が意味を持つ／計算は
1つだけ確かめたいことが多い）。**書かなかった欄は見ない**ので、確かめたいことだけ書けばよい。

決めごと:

- **答えられないことは答えない。** プラグインの計算・検証（アプリが登録するもの）は CLI に
  無いので、値を作らずに `cannot` に並べる（0 や null を返すと「計算した結果が 0」と読める）。
  同じ理由で `source` を持つ明細は畳まない
- **押せるかどうかを答えるのは、レコードを持つ画面だけ**（一覧の上のボタンには判定する相手が
  無い＝`validate` が `enabledwhen-without-record` で言う）。一括のボタンは「選んだ行」で
  判定するもので、シナリオは行の選択を持たないので答えない
- `--draft` の期待は**いまの答えを写したもの**。すぐ回せる代わりに、定義が間違っていれば
  間違ったまま写る＝**業務として正しいかは人が見る**（各件に「何から作ったか」が書いてある）。
  形が決まっている項目（`pattern`）は値を作らず `TODO_<項目>` を置く
- `--cover` が数えるのは**定義の分岐**（条件は成立した側と、しなかった側の両方。検証は通った
  側と落ちた側の両方）。行数では数えない。**落とすための道具ではない**（次に書くシナリオが
  決まるのが値打ち）
...
`roles` は5か所に書けるので、**役割から引く**には定義を全部開くことになる。棚卸しで
...
- **誰でもない人（未ログイン）を必ず1列入れる。** 役割を持つ人だけを並べると、
  ログインしていない人に何が見えているかを誰も見ないままになる
- 並べるのは**役割で絞られている物だけ**（絞っていない物は全員に見えるので、並べると
  表が定義の大きさになって読まれない）
- 「画面」の行は**入口を辿った結果**（ページ自身に `roles` は書けない）。メニューの行は
  同じことを言うので出さない
- `--role` は**定義に出てこない役割名をエラーにする**（黙って0枚にすると「その役割では
  何も開けない」と読めてしまう）。入口が定義に無い単票は絞れないので、別に言う
- 役割ごとの行には「**見えるのは何件・見えないのは何件**」も出る＝「この役割で何が
  できるか」は**見えない側**まで書かないと分からない
- `--registry` を渡すと「**アプリが配るのに、定義が出し分けに使っていない役割**」も言う。
  **消せとは言わない**（アプリのコードで使っていることもある）＝「その役割で見え方が
  変わる所は1つもない」という事実だけ。逆向き（定義にしか無い役割）は `validate` の警告
- ○×は**定義に書いてあること**だけ。API を直接叩けばデータは取れるので、実際の制御は
  バックエンドで守る（`hatake attack` で試せる）
...
**やるのは「行を件に開く」だけ。** まとめない・言い換えない・順番を変えない。

| 機械が決めること | 決めないこと |
|---|---|
| 行を件に開く（箇条書きは1行1件、散文は句点で分ける） | 何を言ったのか（`text` は**その行のまま**） |
| 分類（見出しと合図の言葉だけ） | 見出しが無ければ全部 `asked`（推し量らない） |
| `covers`（**業務の言葉が一致した所だけ**） | 一致しなければ空にして人に振る |

- 見出しの合図: 「決まっていない / 未定 / 未決 / 保留 / TBD」→ `undecided`、
  「決めごと / 制約 / ルール / 規則」→ `decisions`、
  「終わりの判定 / 完了条件 / 受け入れ」→ `acceptance`。それ以外は `asked`
- 行の書き出し（`探す:` / `見る:` / `押す:` / `入れる:`）が**当てる種類を絞る**。同じ
  言葉が絞り込みと列の両方に在るのは普通なので（「商品名」）、これが無いと1件が両方を
  指す。書き出しは[頼み方のテンプレ](../site/docs/asking.md)が決めている字で、
  推し量りではない
- 項目名そのもの（`orderNo`）が指示文に出ていても当てる（API の名前で来ることがある）。
  ただし**短い名前は見ない**（`id` が「〜という id」に当たってしまう）
- テンプレの案内文（`（1文。例: …）`）とコードブロックの中は読まない（貼った定義を
  要求にしない）
- 起こしたものは**全部 `source: ai-draft`**（未確認）。人が読んで `confirmed: true` に
  するまで、「AI がこう読んだ」以上のことは主張しない
- 定義を渡すと、**指示文のどこにも言葉が一致しなかった相手**も挙がる（頼まれていないか、
  言い方が違うだけ）＝勝手に足した項目・ボタンがここで出る
- **入力は指示文だけ。** 定義から要求を起こしてはいけない（起こせば必ず一致するので、
  突き合わせが何も言わなくなる）
...
/まとめて実行できるのに権限が無い・**まとめて実行するのに確認が無い**・**1回で 100 件動くのに区切り
（`batchSize`）が無い**・金額らしいのに桁区切りが無い・明細に親のキーが無い・帳票に合計が無い、を
挙げる。
