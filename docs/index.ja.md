...
| ```` ```yaml ```` の**断片**（`columns:` だけ・`- { field: … }` だけ） | キー名を DSL の語彙（`spec/reference.json`）と（消えたキー・綴り違いが出る）。囲みに `context:<ノード>` と書けば**その場所に書けるか**まで |
...
- 断片の囲みに **`context:<ノード>`**（`context:table` / `context:column` /
  `context:searchPage`）と書くと、キー名だけでなく**その場所に書けるか**まで見る
  （`filter` の下の `columns` は、キーは在るが書けない）。名前は
  `spec/reference.json` のノード名＝`hatake reference <名前>` で引ける。
  **知らない名前は落ちる**し、丸ごとの定義に付けても落ちる（印は1つの意味しか持たない）
- 印の無い断片はキー名だけ。**何枚を書ける場所まで見たか**は毎回出る
