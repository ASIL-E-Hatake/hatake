...
| 説明の次段（差分の説明） | `hatake explain --diff <前> <後>` … 変更を画面の言葉で言う（「請求先の枠が法人のときだけ出るようになった」）。`diff` は機械の言葉で言うので、人のレビューには一段足りない | AI に直させたとき、レビューする人が読むのは「何が変わったか」。いまは api/ui の変更一覧までで、日本語の説明が無い | 中 |
| 失敗例の自動収穫 | `validate` / `explain` を通した記録から、**新しい転び方**を候補として拾う（同じ警告が繰り返し出た定義を匿名化して溜める）。人が選んで `failures.json` に入れる | 実例は手で書くと増えない。実際に転んだ所を機械で集めないと、カタログが古くなる | 中〜大 |
| 説明の英語版 | `explain` の出力を英語でも出す（語彙表を ja/en の2枚にする） | 警告と同じく今は日本語だけ。海外案件や英語話者のレビューには足りない | 小〜中 |
| 定義の要約（1行） | `hatake explain --brief` … 画面1枚を1行で言う。app なら一覧表を出す。README や PR 本文に貼れる形 | 全文は長い。目次として使える短い形が要る（AI がコンテキストに積むときも軽い） | 小 |
