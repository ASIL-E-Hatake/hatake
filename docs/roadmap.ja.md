...
| 一括 | 入力中の値でも出し分ける | `enabledWhen` を書いた画面のボタンが見るのは、いま**開いているレコード**（入力中の値は見ない）。フォームの入力に追従させる | 「下書きに直したら送信できる」が書けない。項目の側（`visibleWhen` / `computed`）は入力に追従しているので、ボタンだけが取り残されている | P2 | 中 |
