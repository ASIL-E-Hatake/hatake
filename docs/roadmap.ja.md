...
| 定義の品質 | MCP サーバ（`hatake-mcp`） | ✅ TS 同梱 | AI エージェントに「仕様の引き当て・例の取得・検証・**助言**・雛形・API の形」を道具として渡す（`hatake_reference` / `hatake_examples` / `hatake_validate` / `hatake_advise` / `hatake_new_page` / `hatake_api_shape` ほか全15個）。**`hatake_advise` は「書いていないこと」を渡す道具**＝書いたものは検証できるが、抜けは検証に出てこない（並べ替えできない一覧も、確認の無い一括も、定義としては正しい）。物差し（`off` / `options` / `require`）もそのまま渡せて、知らない規則名は黙って無視せずエラーにする。**`hatake_apply_advice` は選んだ助言をその場に書き込む口**（当てるかどうかと書く値は呼ぶ側、書く場所と書けたかの確認は機械）。**`description` と実際の引数を突き合わせる検査**も回している（宣言だけあって読まない引数・読むのに宣言の無い引数・説明の中の実在しない道具名は落ちる＝説明が嘘をつけない）。助言には**書く値の下書き**（`draft`）が付き、`hatake_explain` の `roles: true` で**定義に出てくる役割**を引ける（役割名を想像で書かせない）。**依存ゼロで手書き**（stdio の JSON-RPC 2.0 で必要なのは `initialize` / `tools/list` / `tools/call` だけ。CLI と同じ判断で `@modelcontextprotocol/sdk` を入れない）。プロトコル `mcp.ts` と道具 `mcpTools.ts` を分離。名乗るバージョンは `2025-06-18` / `2025-03-26` / `2024-11-05`。**知らない道具はプロトコルのエラー、道具の中の失敗は結果として返す**（後者はモデルに読ませて直させる）。`initialize` の `instructions` に使う順番（例を探す→雛形→キーを引く→検証）を載せてある。CI で stdio の往復を実際に流す。→ [使い方](guide/mcp.ja.md) |
...
読み直すことになる）。並びは**P1 を先頭に**、あとは残りの手数が少ない束（空振り・図）から。

いま P1 は「試す」の1件だけ。**定義ファーストの穴ではない**（定義そのものは書ける）が、
**確かめる側がコードに落ちる**＝AI が定義を書いても、それが動くかどうかを確かめるには
画面の試験を手で書くしかない、という穴。書く道具（`new` / `fix` / `advise`）と読む道具
（`explain` / `diagram` / `paper`）は揃ったのに、**動かす道具**が無い。

| 束 | 何をまとめているか |
|---|---|
| 試す | 定義を**動かして**確かめる（シナリオ・試していない所・画面の試験の口） |
...
| 束 | 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|---|
| 試す | 定義を動かして答えを見る（`run` ＋ シナリオ） | `hatake run <定義> --scenario s.json`＝レコードと操作を渡して、**検証の結果・計算した値・押せるボタン・サーバに送る形**を返す（Renderer もブラウザも要らない）。シナリオは宣言（「この項目にこう入れた → こうなる」）で、3版が同じ答えを出す＝いま枠組みが自分に使っている収束テストの仕組みを、そのまま利用者にも渡す | **AI は定義を書けるが、書いたものが動くかを確かめられない**。`validate` が言うのは「書ける」ことだけで、`explain` が言うのは「そう書いてある」ことだけ。計算が合っているか・その条件で必須になるか・そのボタンが押せるかは、いま**画面の試験を手で書く**しかない（Flutter が要る＝AI にとって一番遠い所）。答えが文字で返れば、書く→確かめる→直すが1コマンドで回る | **P1** | 大 |
| 試す | シナリオの下書きを定義から起こす | `hatake run --draft`＝`required` / `requiredWhen` / `compare` / `unique` / `maxLength` などから「**通る1件**」と「**落ちる1件ずつ**」を作る（境界値も: 文字数ぴったりと1文字超、`compare` の等号、`unique` の2行目） | シナリオの器を渡されても、AI は業務の値を知らない（客先の語彙は定義に無い）。**制約は定義に書いてある**ので、境界のデータは機械が作れる＝人が書くのは「業務としてあり得る値」だけになる | P2 | 中 |
| 試す | 試していない所を数える | `hatake run --cover`＝画面・ボタン・条件（`visibleWhen` / `enabledWhen` / `requiredWhen` / `where`）のうち、シナリオが**一度も通っていない**ものを挙げる | AI は「もう十分書いた」の判断ができない（人も同じ）。定義が分岐を全部知っているので、**次に書くシナリオ**を機械が指せる。カバレッジを行数ではなく**定義の分岐**で数えるのは、定義ファーストだからできること | P2 | 中 |
