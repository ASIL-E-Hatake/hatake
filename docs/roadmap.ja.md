...
| 新しく空いた穴だけを出す | `attack --all-roles` の結果を前回と比べて、**新しく空いた穴**だけを出す（`--since last.json`）。毎晩回す前提の形 | 毎晩回すと出力は毎晩同じ。人は同じ表を読み続けられないので、**変わった所だけ**にする必要がある（`harvest` の常設化と同じ形） | P1 | 小〜中 |
