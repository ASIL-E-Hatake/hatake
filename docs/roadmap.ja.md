...
| 項目 | 内容 | なぜ | 規模感 |
|---|---|---|---|
| 一覧の生成の次段 | 読めない登録（動的に組み立てているもの）を、**実行時に自己申告させる**口（`RepositoryRegistry.keys` を吐くデバッグ用の出力など）。静的に読めないものは、動かして聞くしかない | 走査は「その場に書いてある文字列」しか読めない。プレイグラウンドのように動的に作る登録は原理的に読めない | 中 |
