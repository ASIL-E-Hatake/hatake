...
| 説明の英語版 | `explain` の出力を英語でも出す（語彙表を ja/en の2枚にする） | 警告と同じく今は日本語だけ。海外案件や英語話者のレビューには足りない | 小〜中 |
...
| 説明の読み合わせ（意図との照合） | 頼まれたこと（自然言語）と `explain` の出力を突き合わせて、**言われていないのに在るもの／言われたのに無いもの**を挙げる | 読み返しても、人は「書いてあるもの」を追認しがち。**足りない側**は指摘されないと気づけない | 中〜大 |
| 要約から探す（画面の索引） | `explain --brief` の出力を集めて、「顧客を検索する画面はどれ」に答える索引にする（`counts` で規模順にも並ぶ） | 定義が増えると**どこに何があるか**が分からなくなる。AI に「近い画面」を探させる入口が要る | 小〜中 |
| 図解を定義から作る | いまの図解は手書きのテキスト（`docs/diagrams/*.json`）。**実際の定義から**「この app の画面とメニューと遷移」の図を出す（`hatake diagram app.yaml`） | 画面が増えると遷移が追えなくなる。同じ生成器（`render-diagrams.mjs`）に元データを流し込むだけで済む | 中 |
| 最小の再現から `fixed` を作る | 収穫した最小の再現に対して、**直し方が一意なもの**は直した形まで機械で作る（`fixed` の下書き）。診断がゼロになることを確かめてから出す | 候補で残る人の仕事がもう1つ減る。`--fix` と同じ機械で書ける | 中 |
| 助言の物差しを外から渡す | `advise` の規則は今 TS の中の表。**案件ごとの決めごと**（「一覧には必ず並べ替えを付ける」等）を設定で渡せるようにする（`hatake advise --rules team.json`） | 助言は好みなので、会社・案件で変わる。固定だと「合わないから使わない」になる | 中 |
...
| 助言と説明を1枚にする | `explain` の「この画面でできないこと」と `advise` の「書き足したほうがいい所」は隣の話。レビュー用に1枚で出す（`explain --review`） | レビューする人が見る紙は1枚がいい。道具ごとに出力が散ると読まれない | 小 |
...
| 説明の語彙を spec から引く | `explain` の語彙表（フォーマッタの見え方・演算子の言い方）は今 TS の中の表。**スキーマの `description` から引く**か、対訳表を `spec/` に出して3言語で共有する | 語彙が実装の中にあると、Dart 版で説明を出したいときに二重管理になる（英語版も同じ問題） | 中 |
...
>   **絵は生成物**で、元は `docs/diagrams/*.json`（縦積みの箱と矢印だけの素朴な形式）。手で描くと
