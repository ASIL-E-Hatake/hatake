...
読み直すことになる）。**P1 は入った**ので、ここに残っているのは P2 だけ。並びは、
残りの手数が少ない束（試す・空振り・図）から。

| 束 | 何をまとめているか |
|---|---|
| 試す | 定義を**動かして**確かめる残り（サーバ側・登録の辻褄・画面の試験の口） |
...
| 束 | 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|---|
| 試す | サーバでも同じシナリオを回す | Java 版にも `ScenarioRunner` を置く（TS と Dart には在る）。サーバ側の試験で同じファイルを食べれば、**画面・道具・サーバの3つが同じ答え**であることを案件のシナリオで確かめられる | 検証の一致は収束テストで縛っているが、**案件のシナリオ**でサーバを確かめる道が無い（いまは画面と道具の2つ）。`FormValidator` は既に在るので、glue だけ | P2 | 中 |
| 試す | プラグインの登録が版どうしで揃っているかを言う | `registrySnapshot` に**計算（`ComputedRegistry`）の登録**が入っていない（Repository / プラグイン / 検証 / 変換 / 役割は入っている）。まずそこを足し、次に「Dart には在るが Java には無い計算・検証」を突き合わせて言う | DSL の約束は「同じ定義なら同じ答え」だが、**利用者が足したもの**についてはその約束を確かめる道が無い。`customKeys` は3版に在るので、突き合わせる相手は既にある | P2 | 小〜中 |
| 試す | 画面の試験を書くための口を配る | `hatake_test` パッケージ: 定義から画面を出す `pumpPage(definition, rows: …)`・偽の Repository・**キーの規約**（`hatake.form.<項目>` / `hatake.action.<id>` / `hatake.rowaction.<id>.<鍵>` / `hatake.confirm.ok`）を公開して固定する。デモに埋まっている「項目名からそれらしい値を作る」（`playground_data.dart`）も、デモ専用をやめてここへ出す | 画面の試験は**書ける**（枠組みの試験はそう書いてある）が、キーの規約は公開された契約ではない＝読み解いてから書くことになり、AI は当てずっぽうになる。規約を配れば「押す・入れる・確かめる」を定義の言葉で書ける。規約を変えたら落ちる所も1か所にまとまる | P2 | 中 |
| 試す | サーバ側の試験データを定義から出す | `hatake fixtures <定義>`＝DTO の形と制約から、境界のレコードを JSON で出す（通る形・弾かれる形の両方）。`schema` / `openapi` の隣に置く | 契約は出せるようになった（`readOnly` まで含めて）が、**試すデータ**はサーバを書く人が手で作っている。制約は定義に在るので出せる＝画面とサーバが**同じ境界**で試される（別々に作ると必ずどちらかが緩い） | P2 | 小〜中 |
