...
| アクションの次段（残り） | 一括の**途中経過**（100件を押したときの進み具合）と**中断**。押す前の出し分け（`enabledWhen`）は入った | `confirm` / `prompt` / `onError` / `scope: selection` / `enabledWhen` が入って、残っているのは「長く走るとき」だけ。失敗した行の名指し（`FailedRow`）と繋がる話 | P2 | 中 |
