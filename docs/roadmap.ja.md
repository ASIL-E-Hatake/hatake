...
| 図 | 図を SVG 以外にも | 遷移図を Mermaid / DOT でも出す（`--format mermaid`）。PR 本文や Wiki にそのまま貼れる | SVG は貼れる場所が限られる。**貼れる形**でないと、図は結局共有されない | P2 | 小〜中 |
| 図 | 計算の依存を絵にする | `explain --diagram` に「どの項目がどの項目から出るか」を足す（`小計 → 消費税 → 合計`、明細の行から親へ）。順番の警告が出たときに**どこを動かせばいいか**が1枚で見える | 順番は機械が見るようになったが、直す側は表を目で追うことになる。依存は定義から読めるので、絵にするのは書くだけの仕事 | P1 | 小 |
