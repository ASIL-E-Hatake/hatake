...
| 一括 | 区切りの件数を役割で変える | `batchSize` を `maxRows` と同じ `{ default, byRole }` の形にする（回線の細い拠点は小さく、社内は大きく） | 上限は役割で変えられるのに、区切りは全員同じ。同じ「何件ずつ」の話なので形も揃えたい | P2 | 小 |
| 一括 | 進み具合に残り時間を出す | 区切りごとの所要から見当を出す（「あと 30 秒くらい」）。件数だけだと、100件が1分なのか10分なのか読めない | 待つ人が「席を離れていいか」を決められない。区切りの時間は枠組みが持っている | P2 | 小 |
| 一括 | 中断したところから再開する | 止めたときに「送っていない行」を選び直せるようにする（失敗した行の選び直しと同じ形） | いまは中断したら選択が解ける＝もう一度選び直すのは人の手。送っていない行は枠組みが知っている | P2 | 小 |
