...
| 権限 | 役割ごとの見え方を1枚に並べる | `explain --roles` の隣に「同じ画面を役割ごとに並べた表」（列・ボタン・入口の○×）。デモの札で切り替えるのと同じことを、文字で1枚にする | 切り替えて見せられるようになったが、**まとめて見比べる**にはまだ手で切り替えるしかない。棚卸し（人事異動）で聞かれるのはこの形 | P2 | 中 |
...
| 権限 | 配っているのに使っていない役割を言う | アプリが語彙に書いてあるのに、定義のどこでも出し分けに使っていない役割を出す（`explain --roles` か `registry` の側で。**消せとは言わない**＝定義側の書き忘れの疑い） | 片方向（定義 → アプリ）は入った。逆は「アプリに `viewer` が在るのに、どの画面も `viewer` で何も変えていない」＝出し分けの書き忘れの兆候 | P2 | 小 |
...
| 権限 | 役割ごとの一覧を出す | `hatake index --role admin` … その役割で開ける画面だけの索引（役割の追加・棚卸しに使う） | 権限の棚卸しは「役割から画面」を引く作業。いまは画面から役割しか引けない | P2 | 中 |
| 権限 | 権限の差分を見る | `hatake diff` に**誰が開けるか**の変化を足す（「単価マスタは admin も開けなくなりました」） | いまの diff は画面と契約の変化だけ。権限は**入口を1つ直すと遠くの画面に効く**ので、目で追えない | P2 | 中 |
| 権限 | 権限の棚卸しを1枚で | `explain --roles`（役割ごとに、開ける画面・押せるボタン・見える列を1枚に）。人事異動のたびに聞かれる形 | 「この役割で何ができるか」は画面からではなく**役割から**引く作業。いまは画面→役割しか引けない | P2 | 中 |
