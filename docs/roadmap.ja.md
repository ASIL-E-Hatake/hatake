...
| デモで役割を切り替える | 実行中のデモに役割の切り替えを入れて、**隠れる列・押せないボタン**を目で見せる（`HatakeScope(roles:)` は在るが、デモは何も渡していないので権限が動いている所が見えない） | 例と道具は入ったが、**動いている姿**がまだ無い。権限は「見えないことが正しい」機能なので、切り替えて見せないと伝わらない | P2 | 中 |
...
| 失敗の言い方に助言を出す | `advise` に「`onError` の文言に `{failedKeys}` が無い（行を名指しできるのに件数しか言っていない）」を足す。値の下書きも作れる | 名指しできる口はできたが、**使うかどうかは書く人次第**。書いていないことを言うのは助言の仕事 | P2 | 小 |
| 差し込みを他の文脈にも広げる | 検証のメッセージ（`{value}` / `{target}`）と遷移のパラメータ（`$row.<項目名>`）も、書き間違いを機械が言えるようにする（いま言えるのはボタンの文言だけ） | 一覧は3つの文脈を持てたが、**検査があるのは1つ目だけ**。`$row.存在しない項目` は黙って空になる | P2 | 中 |
| 操作の記録を定義から起こす | 誰が・いつ・どの行に・何をしたか、を残す口（`onSuccess` の隣に `audit`）。中身はアプリ側 | 一括を入れた次に必ず来るのが「誰がやったのか」。監査が要る業務では、これが無いと使えない | P2 | 中 |
| 居座っている穴を出す | 何晩も残っている穴に**日数**を付ける（`--since` が見るのは1晩前だけなので、「ずっと穴のまま」と「昨日直って今日また空いた」が同じに見える） | 変わった所だけは出せるようになったが、**変わらないもの**の中に「誰も直していない穴」が埋まっている。日数が付けば、直す順番が決まる | P2 | 中 |
| 役割に画面の言葉を付ける | 役割名は識別子（`hr` / `manager`）なのに、**画面の言葉で言い直す場所が定義に無い**。`app.roleLabels` のような形で持てるようにする（説明・図・棚卸しがその言葉で出る） | 権限の棚卸しを人に見せる場面で、`hr` を「人事部」と読み替えるのは毎回人の頭の中。定義に1回書けば3つの出力が揃う | P2 | 小 |
| 役割名をアプリ側と突き合わせる | 実装が配っている役割名（`HatakeScope(roles:)`）を読んで、定義の役割名と突き合わせる（`registry` と同じ形＝読めなければ「読めない」と言う） | 定義の中で綴りが揃っていても、アプリが配る名前と違えば誰にも見えない。**画面を見ても気づけない**転び方で、いまは誰も見ていない | P2 | 中 |
| 下書きを実例から作る | 助言の下書きを、定義の中だけでなく**実例カタログ**（`failures` / `harvest`）からも作る（「この案件では確認の文をこう書いている」） | いまの下書きは列名とボタン名から作るので、案件の言い回しは入らない。溜めた実例を使えば、2回目からは案件の言葉になる | P2 | 中 |
| 説明の中の例を機械が実際に呼ぶ | `description` に載せた呼び出し例を CI でそのまま実行して、通ることを見る（いま突き合わせているのは引数名と道具名まで） | 引数の名前が合っていても、**例が古い**ことはある（値の形が変わった・必須が増えた）。AI は例を写すので、例が嘘だと最初の1回で転ぶ | P2 | 小 |
| 物差しを定義の群から起こす | `harvest --rules` … 既にある定義を数えて「この案件では全部の列に `width` を書いている」を見つけ、`require` 規則の下書きにする | 案件の決めごとは、たいてい**もう定義に書いてある**（誰も明文化していないだけ）。物差しを手で書くのは最初の一歩が重い | P2 | 中 |
...
| 失敗した行を紙と CSV に出す | 「どの行が失敗したか」を画面の外へ持ち出す口（失敗した行だけの CSV / 帳票）。いまは画面の中でしか読めない | 一括の失敗は**そのあとの作業**（担当に配る・翌日やり直す）に繋がる。画面に出しただけでは、閉じたら消える | P2 | 中 |
...
| 変わった所を issue の本文にする | `--since` の出力を、そのまま issue / PR コメントに貼れる形で出す（`pr-comment` と同じ形。同じ食い違いは数だけ更新） | 回す形と落とす形は入ったので、残るのは**誰かに届く形**。CI の緑赤だけでは、直す人の手元まで来ない | P2 | 小〜中 |
| 一括の上限も棚卸しに乗せる | `explain --roles` に「その役割の一括上限」を出す（`maxRows.byRole` は定義のあちこちに散るので、役割ごとに何件動かせるかが読めない） | 上限は役割ごとに変えられるようになったが、**役割から引く口が無い**（ボタンごとに定義を読む形）。棚卸しに1行足すだけで読める | P2 | 小 |
| 資格の取り方だけを試す | `--login --check`＝業務の口は叩かず、**資格が取れることだけ**を見る（役割ごとに ok / 401）。CI に置く前と、落ちた晩の切り分けに使う | いまは資格が取れないと「その役割は叩いていません」として結果に混ざる。**資格の話とサーバの話**を切り分ける口が無い | P2 | 小 |
| 食い違いの印から直し方を引く | `probe --json` の `kind`（`type-mismatch` / `list-shape` …）を、直し方の表（`pitfalls` / 実例カタログ）に繋ぐ | 印は「前回と比べる」ために付けたが、**機械が引ける鍵**でもある。警告 → 直し方は繋がっているのに、食い違い → 直し方が繋がっていない | P2 | 小 |
| 違いを機械に渡す | `--since` の結果を MCP の道具にする（`hatake_changed`＝前回の結果と今回の結果を渡すと、変わった所だけ返る） | 毎晩の違いを読むのが人だけなら、直すのも人だけ。**変わった所**は AI にいちばん渡しやすい単位（小さくて、文脈が要らない） | P2 | 中 |
