...
| 例に役割を1つも書いていない | `spec/examples/` と デモアプリに `roles` を使った例が**1件も無い**（役割の棚卸しを走らせると「全部の人に全部見えます」と出る）。1枚に権限を入れる | AI は**例を写して直す**。書き方の例が無いキーは、リファレンスを引いてもらうしかない＝一番よく通る道に穴がある | P1 | 小 |
...
| 役割ごとに開ける画面を並べる | `explain --roles` に「その役割で開ける画面」を足す（いまは**書いてある場所**だけ。開ける画面は `appAccess` が持っている） | 棚卸しの次に来るのは「この役割は結局何ができるのか」。計算は在るので繋ぐだけだが、**書いてある場所と辿った結果**を混ぜない形にする必要がある | P1 | 小 |
