...
| 一括 | 中断した続きを、読み直しても覚えている | いまの選び直しは**画面の中だけ**（検索し直す・ページを変える・閉じると消える）。送っていない行を持ち出せる形（CSV／選択の保存）にする | 「もう一度押せば続く」は同じ画面に居る間の話。1000件を翌日に続けるなら、画面の外に出す口が要る | P2 | 中 |
...
| 一括 | 失敗した行を紙と CSV に出す | 「どの行が失敗したか」を画面の外へ持ち出す口（失敗した行だけの CSV / 帳票）。いまは画面の中でしか読めない | 一括の失敗は**そのあとの作業**（担当に配る・翌日やり直す）に繋がる。画面に出しただけでは、閉じたら消える | P2 | 中 |
