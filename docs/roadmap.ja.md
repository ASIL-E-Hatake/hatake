...
| アプリ側の配線を下書きする | `hatake wire app.yaml --lang dart` … 定義から `HatakeScope(...)` の骨組みを出す（要る Repository の口・`exportSink` / `printSink`・登録が要るプラグインを埋めた形） | 定義は AI に書けても、**繋ぐコード**でつまずく（どこに何を渡すかは仕様書を読まないと分からない）。定義から機械的に決まる部分は出せる | P1 | 中 |
