...
| 変更のたびに説明を貼る（CI の口） | `explain --diff --git` と `--markdown` は入ったので、残りは**PR に貼る所**。GitHub Actions の断片を `docs/guide/` に置く（既にあるコメントを書き換える・変化が無ければ貼らない） | 道具が在っても、**貼る仕掛けが無ければ貼られない**。`harvest` の常設化と同じ話 | P1 | 小 |
| 説明の英語版（Markdown も） | `explain --lang en`。語彙は `spec/vocabulary.json` に `en` があるので、残りは文の組み立て。PR 本文の形と組むと、英語話者のレビューが回る | 貼れる形になった今、次に効くのは言語。語彙が spec に出ているので一番安い | P1 | 中 |
