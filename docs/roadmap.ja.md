...
| 権限 | 役割名をアプリ側と突き合わせる | 実装が配っている役割名（`HatakeScope(roles:)`）を読んで、定義の役割名と突き合わせる（`registry` と同じ形＝読めなければ「読めない」と言う） | 定義の中で綴りが揃っていても、アプリが配る名前と違えば誰にも見えない。**画面を見ても気づけない**転び方で、いまは誰も見ていない | P2 | 中 |
