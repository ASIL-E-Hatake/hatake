...
| 定義の品質 | MCP サーバ（`hatake-mcp`） | ✅ TS 同梱 | AI エージェントに「仕様の引き当て・例の取得・検証・**助言**・雛形・API の形」を道具として渡す（`hatake_reference` / `hatake_examples` / `hatake_validate` / `hatake_advise` / `hatake_new_page` / `hatake_api_shape` ほか全14個）。**`hatake_advise` は「書いていないこと」を渡す道具**＝書いたものは検証できるが、抜けは検証に出てこない（並べ替えできない一覧も、確認の無い一括も、定義としては正しい）。物差し（`off` / `options` / `require`）もそのまま渡せて、知らない規則名は黙って無視せずエラーにする。**依存ゼロで手書き**（stdio の JSON-RPC 2.0 で必要なのは `initialize` / `tools/list` / `tools/call` だけ。CLI と同じ判断で `@modelcontextprotocol/sdk` を入れない）。プロトコル `mcp.ts` と道具 `mcpTools.ts` を分離。名乗るバージョンは `2025-06-18` / `2025-03-26` / `2024-11-05`。**知らない道具はプロトコルのエラー、道具の中の失敗は結果として返す**（後者はモデルに読ませて直させる）。`initialize` の `instructions` に使う順番（例を探す→雛形→キーを引く→検証）を載せてある。CI で stdio の往復を実際に流す。→ [使い方](guide/mcp.ja.md) |
...
| 直し方も AI の道具にする | MCP に `hatake_fix --todo` 相当（直さなかったものを「次の1往復で渡す形」で返す）と、`hatake_advise` の結果をそのまま当てる口を足す。いまは助言を読んで**モデルが手で直す** | 一意な直しは機械の仕事。助言まで渡せるようになったので、次は「読んだ助言を当てる」ところを機械に寄せる | P2 | 中 |
| 道具の説明を機械が見る | `description` は AI 向けの契約なのに、**中身が正しいかを誰も見ていない**（道具名の綴り・引数名・「いつ使うか」）。実際の引数と突き合わせる検査を足す | 道具は14個に増えた。説明と実装がズレると、モデルは黙って使わなくなる（落ちないので気づけない） | P1 | 小 |
