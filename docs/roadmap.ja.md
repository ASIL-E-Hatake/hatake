...
| 索引を Dart / Java でも | 索引の作り方（要約＋探すための語）は TS だけ。定義を持っているのはアプリ側なので、**Flutter の中から自分の画面を探せる**と効く（プレイグラウンドの画面選択にも使える） | 索引は「定義の山を持っている側」で要る道具。TS の CLI が前提だと、アプリからは使えない | 中 |
| 図に遷移の向きを描く | いまの図は段（層）だけで、**どの画面からどの画面へ**は矢印1本にまとめている。箱のあいだに線を引けるようにする（作図器の拡張） | 段だけだと「AとBのどちらから開くのか」が読めない。遷移が増えると効く | 中〜大 |
