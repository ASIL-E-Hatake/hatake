...
| 道具の信頼 | 説明の中の例を機械が実際に呼ぶ | `description` に載せた呼び出し例を CI でそのまま実行して、通ることを見る（いま突き合わせているのは引数名と道具名まで） | 引数の名前が合っていても、**例が古い**ことはある（値の形が変わった・必須が増えた）。AI は例を写すので、例が嘘だと最初の1回で転ぶ | P2 | 小 |
...
| 道具の信頼 | 手引きのコマンドを全部走らせる | いま CI が走らせているのは PR コメントの断片とチュートリアルの定義だけ。`cookbook` / `getting-started` に載せた定義とコマンドも同じ形で回す | 文書に載せたコマンドは、走らせないと必ず腐る（読んだ人の所で初めて落ちる）。仕組みは出来たので、あとは対象を増やすだけ | P2 | 小〜中 |
