...
| 定義の自動修復の次段（`--fix`） | 最小化と同じ「1つ直して条件を確かめて戻す」機械（`shrink.ts`）の上に、**直し方が一意な警告**の修復を載せる（`groupBy` に `sort` を足す・`rowActions` の綴り違い）。触る前に `explain --diff` を見せる | 消す（minimize）はできたので、次は**足す・直す**。同じ門（モデルの一致）が使えないぶん、確かめ方の設計が要る | 中 |
| 最小化の次段（**書き足したほうがいい所**） | 落とす逆。`sortable` を1列も書いていない一覧・`key` の無い明細のように、**書いていないから困る所**を挙げる（消すのではなく足す提案） | `minimize` は「書きすぎ」を直すが、業務システムで多いのは「書き足りない」。同じリファレンスから引ける | 中 |
