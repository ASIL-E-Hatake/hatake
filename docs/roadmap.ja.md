...
| 束 | 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|---|
| 空振り | 一括の相手が居ないボタン | `scope: selection` のボタンを、選べる行が無い状態（表が空・絞り込みで 0 件）で押したときの言い方。いまは 0 件でも押せて、ハンドラに空の配列が渡る | 「押したのに何も起きない」の残り。定義では決まらない（実行時の状態）ので、警告ではなく Renderer 側の話 | P2 | 小 |
| 空振り | 行アクションが行に出ない画面を減らす | `crud` / `master` の行に、組み込み以外の行アクション（`rowActions: [detail]`）も出せるようにする。いまは `search` だけが出せる（`crud` では画面上部に出る） | 同じ書き方が画面の種別で違う所に出るのは、覚えられない。`search` の実装が在るので、寄せるだけ | P2 | 中 |
| 空振り | 空振りを実例カタログに載せる | 今回足した6つの規則を `spec/failures.json` に「こう書いた → こう言われた → こう直した」で載せる（`fix --todo` の手掛かりになる） | 規則は増えたが、**直し方の実例**は無い。カタログに在れば、AI は規則名から直し方を引ける | P2 | 小 |
