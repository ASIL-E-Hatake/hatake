...
| 定義の品質 | 構造の間違いの静的検出 | ✅ TS 同梱 | `hatake validate` が**解析は通るのに意図どおり動かない書き方**を警告する（既定ON・終了コードは変えない／`--warn-as-error` で 1／`--no-warn` で黙る／`--json` に `warnings`）。規則は31個（宣言していない行アクション・存在しないページへの遷移・`home` の行き先・カードの `action`・ページ/アクション id と項目名の重複・条件で使えない演算子・`field` の無い集計・`sort` の無い `groupBy`・列に無い項目の合計・`rowActions`/`validators` の書き方・選択肢の連動・項目間の検証（`compare`）の5つ・開ける人が居ない画面・帳票が紙に入らない2つ・紙の無い画面に置いた印刷ボタン・出す口が繋がっていない・選んだ行に対するボタンの置き場所と型の2つ・埋まらない差し込み（**書ける差し込みは4つだけ**＝項目名を書くと文字で出る）・聞いた値を受け取れないボタン・**押しても何も起きない新規登録のボタン**・**明細の合計の書き間違い5つ**）。素の document を見るので `page:` / `app:` どちらでも動き、**遷移先の検査は `app:` のときだけ**（単票は他のページを知らないので誤検出しない）。警告は対照表の id を持つので `hatake pitfalls <id>` に繋がる。MCP の `hatake_validate` も返す |
...
| 行を並べて1行にする | `computed` の行モードに「畳む」ではなく「並べる」を足す（`{ op: join, field: lines, of: productName, separator: "、" }`）。品名を1行にまとめた要約は帳票と確認画面でよく要る | 縦計（数）は入ったが、**文字をまとめる**口が無い。`concat` は同じレコードの項目用なので、行に対しては別の名前が要る | P2 | 小 |
| 明細を絞ってから畳む | `computed` の行モードに条件（`where`）を足す（「キャンセル行を除いた合計」）。`filters` の条件式がそのまま使える | 実務の合計は「全部の行」ではないことが多い。いま条件を付けるにはプラグインを書くしかない | P2 | 中 |
| 計算の順番を機械が見る | 計算は宣言順に1回なので、**前に書いた計算の結果を後ろで使う**のは動くが逆は動かない（空になる）。`validate` に「後ろの計算結果を前で使っている」を足す | 順番の間違いは画面では空欄に見えるだけで、原因が「順番」だと気づけない。依存の向きは定義から読める | P1 | 小 |
