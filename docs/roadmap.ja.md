...
>   残らない）。CI と `mcpExamples.test.ts` が**17個ぜんぶ実際に呼ぶ**。引数の名前が
...
読み直すことになる）。**「試す」の4件と「設計書」の P1・P2 は入った**（動かす・
確かめる側と、言ったことを残す側が一通り揃い、最初の1枚も指示文から起こせる）。
