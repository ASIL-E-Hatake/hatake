...
| 定義の品質 | MCP サーバ（`hatake-mcp`） | ✅ TS 同梱 | AI エージェントに「仕様の引き当て・例の取得・検証・**助言**・雛形・API の形」を道具として渡す（`hatake_reference` / `hatake_examples` / `hatake_validate` / `hatake_advise` / `hatake_new_page` / `hatake_api_shape` ほか全15個）。**`hatake_advise` は「書いていないこと」を渡す道具**＝書いたものは検証できるが、抜けは検証に出てこない（並べ替えできない一覧も、確認の無い一括も、定義としては正しい）。物差し（`off` / `options` / `require`）もそのまま渡せて、知らない規則名は黙って無視せずエラーにする。**`hatake_apply_advice` は選んだ助言をその場に書き込む口**（当てるかどうかと書く値は呼ぶ側、書く場所と書けたかの確認は機械）。**`description` と実際の引数を突き合わせる検査**も回している（宣言だけあって読まない引数・読むのに宣言の無い引数・説明の中の実在しない道具名は落ちる＝説明が嘘をつけない）。**依存ゼロで手書き**（stdio の JSON-RPC 2.0 で必要なのは `initialize` / `tools/list` / `tools/call` だけ。CLI と同じ判断で `@modelcontextprotocol/sdk` を入れない）。プロトコル `mcp.ts` と道具 `mcpTools.ts` を分離。名乗るバージョンは `2025-06-18` / `2025-03-26` / `2024-11-05`。**知らない道具はプロトコルのエラー、道具の中の失敗は結果として返す**（後者はモデルに読ませて直させる）。`initialize` の `instructions` に使う順番（例を探す→雛形→キーを引く→検証）を載せてある。CI で stdio の往復を実際に流す。→ [使い方](guide/mcp.ja.md) |
...
| 助言に値の下書きを添える | `advise` が「何を書くか」の候補まで返す（絞り込みに出す列の候補・確認の文の雛形・合計にする項目）。いまは規則名と「何を足すか」だけで、値は毎回人が考える | 当てる口ができたので、残っているのは**値を決める往復**。定義から作れる下書き（列の名前・金額らしい項目）は機械が出せる。決めるのは人のまま | P1 | 中 |
| 当てた助言を画面の言葉で言い直す | `advise --apply` の結果を `explain --diff` に通して「何が変わったか」を人の言葉で返す（いまは「`page.actions[0].confirm` に … を書きました」＝機械の言葉） | 当てたものを人がレビューする所で止まる。定義の道ではなく「一括承認を押すと確認が出るようになった」で読みたい | P1 | 小 |
