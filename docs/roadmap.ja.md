...
| 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|
| **定義から刷る（`type: print`）** | 印刷を**定義側から起こす**口。`action` の型 `print` と `HatakeScope(printSink:)`（`exportSink` と同じ形）。画面の帳票に印刷ボタンが出る | アダプタは入ったが、**呼ぶのはいまアプリのコード**。CSV は定義から出せるのに印刷は出せない、という非対称が残っている | P0 | 小〜中 |
...
| 印刷の TS / Java 版 | `PrintLayout` までを3言語に（`writePdf` は Dart のまま） | 中立な形があるので後から足せる。**急がない**＝帳票を刷るのは画面かバッチで、そこは Dart | P2 | 中 |
| テーマの次段 | ページ単位の上書き・複数テーマの切替（ダーク手動切替）・`config` で Renderer 固有の見た目を足す口の整備 | 1枚目が出た後の要望。今は app 単位＋`config` 止まり | P2 | 小〜中 |
| アクションの次段 | `onError`（失敗時の文言差し替え）・実行前フック（入力を足す小さなダイアログ）・**複数レコードへの一括実行** | `confirm` / `onSuccess` で足りない所から。一括操作は業務でよく来る | P1 | 中 |
...
| Web URL 同期 / タブ | ルートと URL の相互反映、複数タブ | Web で配ると必ず「URL 共有できないの？」になる | P1 | 中 |
...
>   無い・金額らしいのに桁区切りが無い・明細に親のキーが無い・帳票に合計が無い、の8つ。
...
| 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|
| diff の履歴 | 2ファイルではなく **git の2リビジョン**を比べる（`hatake diff --git HEAD~1..HEAD`）。PR に貼れる Markdown 出力 | 「変更前のファイル」を手で用意するのが面倒で、CI に置きにくい | P1 | 小〜中 |
...
| 説明を PR 本文の形で出す | `explain --diff --markdown` … 見出し・箇条書き・折りたたみ付きの Markdown。CI が PR にそのまま貼れる | いまの出力は端末向け。**人が読む場所は PR** なので、貼る形が要る。`diff の履歴` と組めば「変更のたびに画面の言葉で説明が付く」 | P1 | 小 |
...
| 説明の英語版（語彙は済み） | `explain --lang en`。語彙は `spec/vocabulary.json` に `en` があるので、残りは**文の組み立て**（助詞と語順が日本語前提になっている所）だけ | 海外案件・英語話者のレビュー。語彙が spec に出た今、一番安く効く1件 | P1 | 中 |
| 索引から「使われていない Repository」を出す | 索引は Repository 名を持っているので、`registry` の一覧と突き合わせれば**登録したのに誰も使っていない**が出る（逆向きの `refs`） | 消し忘れた登録は、次に読む人が「使われている」と誤解する。片方向の突き合わせしか無い | P1 | 小 |
| 直せなかったものを AI に渡す形 | `fix` の `skipped` を、そのまま MCP の次の指示にできる形にする（「この3件だけ考えて直せ」）。直った分は文脈から外す | 一意な直しは機械、残りは AI という分担を**1往復で**回したい。いまは人が読んで渡し直している | P1 | 小〜中 |
...
| 権限の説明を `explain` にも | `explain` に「この画面を開けるのは誰か」の節を足す（いまは図だけが知っている）。`--review` の紙にも乗る | レビューは1枚の紙で読む。**図を開かないと分からない**のは、レビューの流れが切れる | P1 | 小 |
...
| 項目間の検証を助言でも勧める | 日付が2つ（`startDate` / `endDate`）・合計と明細が揃っている画面に「`compare` を足しませんか」と言う（`advise` の規則。名前からの推測なので `guess`） | 書けるようになっても**書き忘れる**。定義の形から気づけるものは機械に言わせたい | P1 | 小 |
| 検証の失敗例を実例カタログに入れる | `compare` の5つの警告を `spec/failures.json` に「こう書いた → こう言われた → こう直した」で載せる（`harvest` の対象にもなる） | 新しい書き方は**最初の1回で必ず転ぶ**。転び方が載っていれば、AI も人も1回で直せる | P1 | 小 |
| `explain --review` に権限の節 | 「この画面を開けるのは誰か」を1枚の紙に入れる（いまは図だけが知っている） | レビューは紙で読む。図を開かないと分からないのは、レビューの流れが切れる | P1 | 小 |
...
| 紙に入らない定義を警告する | `validate` が「A4 縦なのに列幅の合計が紙幅を超える」「`rowsPerPage` が多すぎて字が 5pt になる」を言う（いまはアダプタが黙って縮めて収める） | **縮めて収める**は事故を隠す。刷ってから「読めない」と分かるのが一番遅い。数は定義から出るので、刷る前に言える | P1 | 小 |
| AI に紙を見せる | 定義と数行のデータから刷って、**PDF から抽出したテキスト**を返す MCP の道具（`hatake_print_preview`）。列の並び・小計の位置・切れた文字が文字列で読める | AI は画面も紙も見られない。「書いた定義が紙でどう見えるか」を**文字で**返せば、自分で気づける（画像より安い） | P1 | 小〜中 |
...
| チュートリアル | 「0から受注入力画面まで」を通しで1本（今は導入＋レシピが個別にある状態） | 最初の30分の体験が採用を決める | P1 | 小〜中 |
