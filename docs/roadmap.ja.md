...
| 定義の品質 | 構造の間違いの静的検出 | ✅ TS 同梱 | `hatake validate` が**解析は通るのに意図どおり動かない書き方**を警告する（既定ON・終了コードは変えない／`--warn-as-error` で 1／`--no-warn` で黙る／`--json` に `warnings`）。規則は24個（宣言していない行アクション・存在しないページへの遷移・`home` の行き先・カードの `action`・ページ/アクション id と項目名の重複・条件で使えない演算子・`field` の無い集計・`sort` の無い `groupBy`・列に無い項目の合計・`rowActions`/`validators` の書き方・選択肢の連動・項目間の検証（`compare`）の5つ・開ける人が居ない画面・帳票が紙に入らない2つ・紙の無い画面に置いた印刷ボタン・出す口が繋がっていない・選んだ行に対するボタンの置き場所と型の2つ・埋まらない差し込み）。素の document を見るので `page:` / `app:` どちらでも動き、**遷移先の検査は `app:` のときだけ**（単票は他のページを知らないので誤検出しない）。警告は対照表の id を持つので `hatake pitfalls <id>` に繋がる。MCP の `hatake_validate` も返す |
...
| 実行前フック | 押したあと、実行の前に**入力を1つ足す**小さなダイアログ（「承認日を入れてから承認」「却下の理由を書いてから却下」）。`confirm` の一段上で、`form` の一段下 | `confirm` / `onSuccess` / `onError` / `scope: selection` は入った。残っているのはここで、業務では「理由を書かせてから」が普通に来る | P1 | 中 |
...
| 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|
| 危ない一括を機械が言う | `advise` に「`scope: selection` のボタンに `confirm` が無い」「`roles` で絞っていない」を足す（一括は1回の操作で件数ぶん壊れる） | 助言は好みの話だが、**一括だけは既定で厳しくていい**。いまの `open-dangerous-action` は型だけを見ていて、範囲（1件か全件か）を見ていない | P1 | 小 |
...
| 配線の下書きを更新できるようにする | `hatake wire --merge`（既にある配線を読んで、**足りない登録だけ**を足す。手で書いた中身は残す） | いまは再生成すると手で書いた分が消えるので、2回目以降は使われない。差分だけ足せれば、画面を増やすたびに使える | P1 | 中 |
...
| API の食い違いを実際に叩いて見る | `hatake probe app.yaml --base http://localhost:8080/api` … 定義から**要求の形**を組んで実際に叩き、返ってきた形と `openapi` の宣言を突き合わせる（足りない項目・型違い・`totalCount` 無し） | いま縛れているのは「クライアントが宣言どおり送る」ことだけ。**サーバが宣言どおり返すか**は、動かして初めて分かる | P1 | 中 |
...
| 権限の穴を突く | `hatake attack app.yaml --role staff --base …` … その役割で**画面から見えない**はずの口を叩いて、API が実際に拒否するか確かめる | `roles` は画面の出し分けだけ、と何度書いても、遮断を忘れたまま出る。**機械が試せば忘れられない** | P1 | 中 |
