...
| 道具の信頼 | 道具の答えと画面の答えを、案件の定義で突き合わせる | デモのシナリオを `hatake run`（TS）と Flutter の widget テストの**両方**で回し、答えが一致することを CI で見る（いまは収束フィクスチャだけ＝枠組みの側の話） | 「同じ定義なら同じ答え」を縛っているのは枠組みが書いた表だけ。案件の定義（デモ）で縛れば、画面の実装が答えを変えたときに落ちる | P2 | 中 |
...
| 権限 | デモで役割を切り替える | 実行中のデモに役割の切り替えを入れて、**隠れる列・押せないボタン**を目で見せる（`HatakeScope(roles:)` は在るが、デモは何も渡していないので権限が動いている所が見えない） | 例と道具は入ったが、**動いている姿**がまだ無い。権限は「見えないことが正しい」機能なので、切り替えて見せないと伝わらない | P2 | 中 |
