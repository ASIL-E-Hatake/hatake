...
| 定義の品質 | MCP サーバ（`hatake-mcp`） | ✅ TS 同梱 | AI エージェントに「仕様の引き当て・例の取得・検証・**助言**・雛形・API の形」を道具として渡す（`hatake_reference` / `hatake_examples` / `hatake_validate` / `hatake_advise` / `hatake_new_page` / `hatake_api_shape` ほか全16個）。**`hatake_advise` は「書いていないこと」を渡す道具**＝書いたものは検証できるが、抜けは検証に出てこない（並べ替えできない一覧も、確認の無い一括も、定義としては正しい）。物差し（`off` / `options` / `require`）もそのまま渡せて、知らない規則名は黙って無視せずエラーにする。**`hatake_apply_advice` は選んだ助言をその場に書き込む口**（当てるかどうかと書く値は呼ぶ側、書く場所と書けたかの確認は機械）。**`hatake_run` は定義を動かす口**（検証エラー・計算した値・隠れている項目・いま必須の項目・押せるボタン。`draft` で下書き、`cover` で「まだ試していない分岐」）。**`description` と実際の引数を突き合わせる検査**も回している（宣言だけあって読まない引数・読むのに宣言の無い引数・説明の中の実在しない道具名は落ちる＝説明が嘘をつけない）。助言には**書く値の下書き**（`draft`）が付き、`hatake_explain` の `roles: true` で**定義に出てくる役割**を引ける（役割名を想像で書かせない）。**依存ゼロで手書き**（stdio の JSON-RPC 2.0 で必要なのは `initialize` / `tools/list` / `tools/call` だけ。CLI と同じ判断で `@modelcontextprotocol/sdk` を入れない）。プロトコル `mcp.ts` と道具 `mcpTools.ts` を分離。名乗るバージョンは `2025-06-18` / `2025-03-26` / `2024-11-05`。**知らない道具はプロトコルのエラー、道具の中の失敗は結果として返す**（後者はモデルに読ませて直させる）。`initialize` の `instructions` に使う順番（例を探す→雛形→キーを引く→検証）を載せてある。CI で stdio の往復を実際に流す。→ [使い方](guide/mcp.ja.md) |
...
読み直すことになる）。**「試す」の4件と「設計書」の P1 は入った**（動かす・確かめる
側と、言ったことを残す側が一通り揃った）。残っているのは P2 以降だけで、並びは残りの
手数が少ない束（設計書・試す・空振り・図）から。
「設計書」と「試す」に残っているのは、どちらも道具を入れて初めて見えた続き＝**最初の
1枚／1本を人が書いている**所（意図の下書き・設計書の刷り出し・試験コードの下書き）。

| 束 | 何をまとめているか |
|---|---|
| 設計書 | 言ったことを残す道具の続き（下書き・1枚に刷る・変更の由来） |
...
| 束 | 項目 | 内容 | なぜ | 優先度 | 規模感 |
|---|---|---|---|---|---|
| 設計書 | 指示文から意図の下書きを起こす | `hatake intent --draft`（または MCP の `hatake_intent`）＝会話や指示文を渡すと、AI が読み取った要求を `source: ai-draft` で並べた1枚を出す。人はそれを直すだけ＝**コードを読まずに認識のズレが見える** | 意図の器は入ったが、**最初の1枚を書くのは今も人**。指示文はあるので機械が起こせる。ただし要求を定義から生成してはいけない（必ず一致して無意味になる）ので、**入力は指示文だけ**にすること | P2 | 中 |
...
| 読み返し | 説明の読み合わせ（意図との照合） | 頼まれたこと（自然言語）と `explain --review` の出力を突き合わせて、**言われていないのに在るもの／言われたのに無いもの**を挙げる（MCP の道具にする） | 読み返しても、人は「書いてあるもの」を追認しがち。**足りない側**は指摘されないと気づけない。1枚にはなったが、読む人はまだ人間 | P2 | 中〜大 |
