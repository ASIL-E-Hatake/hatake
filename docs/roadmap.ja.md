...
| 権限 | 一括の上限も棚卸しに乗せる | `explain --roles` に「その役割の一括上限」を出す（`maxRows.byRole` は定義のあちこちに散るので、役割ごとに何件動かせるかが読めない） | 上限は役割ごとに変えられるようになったが、**役割から引く口が無い**（ボタンごとに定義を読む形）。棚卸しに1行足すだけで読める | P2 | 小 |
...
| 一括 | 待たせるのに区切りが無いことを言う | `advise` に「1回で 50 件以上動くのに `batchSize` が無い」を足す（進み具合も残り時間も中断も出ない＝押した人は待つしかない）。下書きの件数は `maxRows` から出せる | 区切りは書けるようになったが、**書いていない所**は検証に出てこない（好みなので助言の仕事）。一番待たされる形が既定のままになりやすい | P2 | 小 |
| 一括 | 役割ごとの区切りも棚卸しに乗せる | `explain --roles` に「その役割の区切り」を出す（上限の隣）。`batchSize.byRole` は定義のあちこちに散る | 上限と同じ話。**役割から引く口**が無いと、拠点ごとの件数が合っているかを定義を読んで確かめることになる | P2 | 小 |
| 一括 | 何回に分かれるかを読み返しに出す | `explain` に「100 件選ぶと 20 件ずつ＝5回に分かれる」まで出す（回数は上限と区切りから決まる） | 「1回で何件動くか」は言えるようになったが、**何回サーバを叩くか**は読めない。API の上限（1リクエストの件数）と突き合わせるのは人の頭の中 | P2 | 小 |
