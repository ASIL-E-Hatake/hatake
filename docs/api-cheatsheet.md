...
* **The per-press limit is declarable**: `maxRows: 20` on a `scope: selection` action. While more rows are picked the button is disabled and its label says the limit; it never truncates the selection. Omit it and the real limit is the page size (every row when paging is off).
