...
| **合計が黙って 0 円になる** | `computed` に `fields` も `field` も無い。空欄なら気づけるが **0 は正しく見える**（`computed-without-source` が言う） |
| `of` に項目を並べたのに効かない | `of` は**明細の行のどの値を見るか**（項目名ひとつ）。同じレコードの項目を足すのは `fields` |
