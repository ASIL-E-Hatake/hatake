...
## 散文の中の定義とコマンドを、機械に読ませる

散文には**定義の断片**と**コマンド**が載る。載せたものは走らせないと必ず腐る
（キーが消えた・規則名が変わった・旗が無くなった、が黙って通る）。読む人がいちばん
多いのはサイトなので、ここが腐ると一番効く。

道具はフレームワーク側が持っている（`docs/**` と MCP の例は既にこれで読んでいる）。
**ディレクトリを渡せる**ので、散文もそのまま読める:

```bash
cd typescript && npm ci && npm run build
npm run check-docs -- ../site/prose
```

見るのは3つ。**丸ごとの定義**（`dsl_version:` / `page:` / `app:` で始まる囲み）は
`validate` にかける／**断片**（`columns:` だけの塊）はキー名を DSL の語彙と突き合わせる
／**コマンド**は旗が `--help` に在るかを見て、**引く道具は実際に走らせる**
（`reference` / `examples` / `pitfalls` / `rules` / `failures` / `where`）。

書き方の決めごとは `docs/**` と同じ:

- hatake の定義ではない YAML（`pubspec.yaml` / GitHub Actions）は、囲みの言語のうしろに
  `no-check:<理由>` と書く。**理由の無い `no-check` は落ちる**（黙って外せる印にしない）
- **囲みの中に囲みの字を書かない**（抜き出す側がそこで切るので、載せたものが途中で終わる）
- `<キー名>` のような**置き換える所**が書いてあるコマンドは走らせない（理由つきで
  「走らせなかった」に出る）
- 断片の囲みに **`context:<ノード>`** と書くと、キー名だけでなく**その場所に書けるか**まで
  見る（下）

### 断片は、書ける場所まで見られる

キー名だけの突き合わせでは「キーは在るが、その場所には書けない」が通ってしまう
（`filter` の下の `columns` は、読んだ人の所で初めて効かないと分かる）。足りないのは
**囲む文脈**の1語だけなので、囲みに書く:
...
- 書ける名前は `spec/reference.json` の**ノード名**（`table` / `column` / `field` /
  `searchPage` / `document` …）。`hatake reference <名前>` で引ける
- 要素が並ぶ断片（`- { field: … }` だけ）は、**その要素のノード**を書く（`context:column`）
- **知らない名前は落ちる**（印を付けたまま誰も見ていない、を作らない）
- 印の無い断片は今までどおりキー名だけ。出力に「何枚を書ける場所まで見たか」が必ず出る
- 丸ごとの定義に印は要らない（`validate` にかけているので、書くと落ちる）

**回すのはサイト側**。フレームワークの CI では回していない＝散文を直したときに
こちらの CI が落ちるのは、境界を跨ぐことになるため。サイト側の CI に置くかどうかは
サイト側の判断（置くなら上の3行）。
