...
値**（保存前。計算した項目も含み、`{ mode: create }` も判定できる）、読むだけの画面（`detail`）は
いま開いているレコード。一覧の上のボタンには相手が無いので効かない（`validate` が
`enabledwhen-without-record` で言う）。押せないボタンは**灰色で残り、何の状態で決まるのかが
その画面の見出しで出る**（文言は書かない）。`roles`（見えるかどうか）とは別の話。
...
**まとめて実行する**なら `scope: selection`（既定は `page`）。表にチェックボックスが出て、選ぶまで押せない。**押せない理由はラベルに出る**（「（行を選んでください）」＝まだ選んでいない／「（行がありません）」＝選べる行が無い／選んだら「（3 件）」）。ハンドラは選んだ**行そのもの**を `ctx.records` で受け取り、**呼び出しは1回**（API も1回で済ませられる）。行が入れ替わったら選択は消え、実行できたら解ける。実行できるのは `type: plugin` だけで、**一括の削除は無い**（取り消せない操作は事故が件数ぶん大きくなる）。
...
**一括だけは既定で厳しい**。`advise` が言うのは6つ: 確認が無い（`prompt` があればそれが確認）・確認に件数が無い・失敗の言い方が無い・戻せない名前なのに `danger` が無い・1回で動く件数が多すぎる（ページ送りを切っていると全件）・**1回で 100 件動くのに区切り（`batchSize`）が無い**（進み具合も残り時間も出ず、途中で止められない。下書きの件数は「1回で動く件数を5回に分ける件数」から作る）。`roles` で絞っていない一括は型に関わらず「誰でも押せる危ないボタン」に数える（`advise` も `explain --roles` も同じ見方）。

`explain --roles` の棚卸しには**役割ごとの一括の件数**も出る（「1回 100 件まで・10 件ずつ（上限まで選ぶと 10 回に分かれる）」）。上限も区切りも役割で変わるので、役割から引けないとボタンごとに定義を開くことになる。区切りが無いものは「区切りなし」と出る。`explain` の読み返しにも**何回に分かれるか**が出る（サーバを何回叩くか）。
...
**行のボタンはどの画面の種別でも同じ所（行の右端）に、並べた順に出る**（`search` / `crud` / `master`）。`crud` / `master` では組み込みの `edit` / `delete` も同じ列に混ざる。押した行は `ctx.record` で渡る。**一括のボタン（`scope: selection`）は行には並べられない**（押した行ではなくチェックした行に実行することになるので、行には出さず一覧の上に出す。`validate` が `selection-as-rowaction` と言う）。
...
* 一括に `batchSize: 20` を書くと**枠組みが20件ずつ渡す**＝進み具合（「12 / 100 件」）と
  **残り時間の見当**（「あと 30 秒くらい」＝実測から出すので、1区切り終わるまでは出ない）が
  出て、区切りで**中断**できる（1回で全部渡すと途中の状態は枠組みに分からないので、そこには
  出ない）。中断は「まだ送っていない分を送らない」だけで、送った分は動いている＝文言の
  `{skipped}` に入る。止めた実行は成功ではないので `onSuccess` は動かない。
  **終わっていない行は選んだまま**なので、もう一度押せば続きから動く（区切りが失敗した
