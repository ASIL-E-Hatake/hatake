...
| `hatake_ask` | **書けたあと**：定義に書けないのに決まっていないこと（排他・採番・論理削除・端数・サーバ側の検証・止めるのは誰か）を問いにして返す。**返った問いは人に投げる**＝当てて書くと嘘の設計になる | `source`、`project`（前書き＝もう答えた問い・既定のままでよいと決めた問いは出ない。案件が足した問いも一緒に返る） |
| `hatake_ask`（`impact`） | **触る前に聞く**：その項目を消す／名前を変えると定義のどこが壊れるかを辿る（キー・列・絞り込み・入力欄・計算・条件・遷移のパラメータ・帳票。明細の中まで）。**定義に無い名前は「影響なし」と言わない**。`price:unitPrice` の形で**前:後**を渡すと、そこを全部書き換えた**下書き**（`draft`）まで返る＝当てるのは人。ラベルや文言の中の同じ言葉は触らない（業務の言葉）。**定義の外は直さない**。新しい名前がもう使われていたら落ちる | `source`、`impact`（項目名、または `前:後`） |
...
`tools/list` が返す説明の最後に、**そのまま渡せる引数**が1組付いている。

```
呼び方の例（そのまま渡せます）: {"source":"page:\n  type: crud\n  ..."}
```

これは文で書いた例ではなく、**道具が持っている値そのもの**。CI が全部の道具について
その例で実際に呼び、通ることを見ている（`mcpExamples.test.ts`）。だから**古い例が
残らない**＝AI が例を写して最初の1回で転ぶ、が起きない。

引数の名前が合っていることは前から見ていた（`mcpContract.ts`）。それでも「名前は合って
いるのに値の形が変わっていた」は起きるので、**呼んで確かめる**ところまで機械にした。
