...
| `advise`（書き足したほうがいい所） | 助言は**好み**なので、渡すと AI は全部やる。何を足すかは人が選ぶ。物差し（`--rules`）も案件の決めごとなので、渡す先は人 |
| `explain --review`（レビュー用の1枚） | 中に助言が入っている＝上と同じ理由。読み返しだけなら `hatake_explain` で足りる |
