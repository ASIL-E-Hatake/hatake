...
```
[type-mismatch] 項目の型が約束と違う（数を文字で返している等）。桁区切り・小計・合計が静かに効かなくなる。
  サーバを直すなら: 宣言の型で返す（BigDecimal を文字列で出す実装でよく出る）。
  定義を直すなら: 実物に寄せるなら定義の型を直す。どちらを直すかは業務の判断。
  次に見る: hatake types page.yaml --lang ts
```

**サーバ側と定義側を必ず分けて出します**（どちらを直すかは業務の判断なので、道具は
決めません）。印を省くと16個ぜんぶ出ます。表（[`spec/probe-kinds.json`](../../spec/probe-kinds.json)）
は印の集合と両方向で突き合わせているので、**印だけ増やすことはできません**。
...
役割が増えるたびに全部やり直すのは人が続けられません。**1つ足したときに他が緩んで
...
```json
{
  "$comment": "役割ごとの資格。誰でもない人は資格を送らないので書きません。",
  "accounts": {
    "hr": { "token": "…" },
...
**資格は役割ごとに要ります。** 1つの資格で全部の役割を判定すると、返ってきた 200 が
「その役割でも見えてしまう」なのか「いま使った資格なら正しい」なのか区別できません
（穴が在ることにも無いことにもできてしまう）。なので `--all-roles` に `--token` は
渡せません。資格を書いていない役割は**叩かず、理由を残します**（黙って飛ばすと、
並べた表が「全部 ok」に見える）。

**誰でもない人（資格なし）は毎回1本入ります。** 権限の穴でいちばん多いのが
「ログインしていない人に見えている」なので。ただしその行の「要ログイン」は食い違い
ではありません（定義は「ログインが要るか」を書けないので、そこは判定しません）。
...
```
資格の取り方を試しました（**業務の口は叩いていません**）:
  ・admin … 取れました
  ・staff … 取れません（資格が取れませんでした（401 が返りました））
```

業務の口は1つも叩かないので、**資格の話とサーバの話が混ざりません**（資格が取れない
役割は、叩いた結果では「叩いていません」に混ざってしまう）。1つでも取れなければ終了
コード 1。取れたトークンは出しません。
